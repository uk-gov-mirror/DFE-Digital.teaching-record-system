    ...
    ILogger<TrsDataSyncHelper> logger)
{
    ...
    public async Task SyncPersonAuditsAsync(IReadOnlyCollection<Guid> contactIds, CancellationToken cancellationToken)
    {
        ...
        var processes = (Processes: new List<Process>(), Events: new List<(IEvent Event, Guid ProcessId)>());
        ...
        foreach (var id in contactIds)
        {
            try
            {
                ...
                if (contactAudits.Any(a => a.AuditRecord.ToEntity<Audit>().Action == Audit_Action.Create))
                {
                    AddCreatedEvent(versions.First());
                }
                else
                {
                    AddImportedEvent(versions.First());
                }
            ...
            {
                throw new Exception($"Error syncing audits for Contact with ID {id}", ex);
            }
        }

        // If we've got the same process ID in this batch, stick with the PersonMergingInDqt process type
        var duplicateProcessIds = processes.Processes.GroupBy(p => p.ProcessId).Where(g => g.Count() > 1).Select(g => g.Key).ToArray();
        processes.Processes.RemoveAll(p => duplicateProcessIds.Contains(p.ProcessId) && p.ProcessType == ProcessType.PersonUpdatingInDqt);
        ...
        async Task SyncEventsAsync()
        {
            ...
            foreach (var (@event, processId) in processes.Events)
            {
                writer.StartRow();
                writer.WriteValueOrNull(@event.EventId, NpgsqlDbType.Uuid);
                writer.WriteValueOrNull(processId, NpgsqlDbType.Uuid);
        ...
        void AddCreatedEvent(EntityVersionInfo<Contact> snapshot)
        {
            ...
            processes.Processes.Add(action);
            processes.Events.Add((@event, action.ProcessId));
        }

        void AddImportedEvent(EntityVersionInfo<Contact> snapshot)
        {
            var action = new Process
            {
                ProcessId = snapshot.Id,
        ...
        async Task AddUpdatedEventAsync(EntityVersionInfo<Contact> snapshot, EntityVersionInfo<Contact> previous)
        {
            if (snapshot.ChangedAttributes.Contains(Contact.Fields.StateCode))
            {
                ...
                if (snapshot.Entity.StateCode == ContactState.Inactive)
                {
                    ...
                    // If this is a deactivation as part of a Merge, update the PersonUpdatingInDqt Process and make it a PersonMergingInDqt process instead
                    if (snapshot.Entity.MasterId?.Id is Guid masterId)
                    {
                        var auditsForMasterId = (await GetAuditRecordsFromAuditRepositoryAsync(
                            Contact.EntityLogicalName,
                            Contact.PrimaryIdAttribute,
                            [masterId],
                            cancellationToken))[masterId];

                        var masterUpdatingAudit = auditsForMasterId.AuditDetails
                            .Select(a => a.AuditRecord.ToEntity<Audit>())
                            .SingleOrDefault(a => Math.Abs((a.CreatedOn!.Value - snapshot.Timestamp).TotalSeconds) <= 5 && a.Action == Audit_Action.Merge);

                        if (masterUpdatingAudit is not null)
                        {
                            processId = masterUpdatingAudit.Id;
                        }
                    }
            ...
            processes.Events.Add((
                new PersonUpdatedInDqtEvent
                {
                    ...
                    {
                        Trn = previous.Entity.dfeta_TRN,
                        FirstName = previous.Entity.FirstName,
                        MiddleName = previous.Entity.MiddleName,
                        LastName = previous.Entity.LastName,
                        DateOfBirth = previous.Entity.BirthDate.ToDateOnlyWithDqtBstFix(isLocalTime: false),
                        EmailAddress = previous.Entity.EMailAddress1,
                        NationalInsuranceNumber = previous.Entity.dfeta_NINumber,
                        Gender = previous.Entity.GenderCode?.ToGender(),
                        DateOfDeath = previous.Entity.dfeta_DateofDeath.ToDateOnlyWithDqtBstFix(isLocalTime: true),
                        QtsDate = previous.Entity.dfeta_QTSDate?.ToDateOnlyWithDqtBstFix(isLocalTime: true),
                        EytsDate = previous.Entity.dfeta_EYTSDate?.ToDateOnlyWithDqtBstFix(isLocalTime: true),
                        QtlsDate = previous.Entity.dfeta_qtlsdate.ToDateOnlyWithDqtBstFix(isLocalTime: false),
                        QtlsStatus = MapQtlsStatus(previous.Entity),
                        InductionStatus = MapInductionStatus(previous.Entity.dfeta_InductionStatus),
                        DqtInductionStatus = previous.Entity.dfeta_InductionStatus?.GetMetadata().Name
                    }
                },
                snapshot.Id));
        }
        ...
        where TEntity : Entity
    {
        ...
        {
            audit.AuditDetail.NewValue ??= new Entity();
            audit.AuditDetail.OldValue ??= new Entity();
        }
        ...
        TEntity GetInitialVersion()
        {
            ...
            else
            {
                ...
                {
                    a.AuditDetail.NewValue ??= new Entity();
                    a.AuditDetail.OldValue ??= new Entity();
