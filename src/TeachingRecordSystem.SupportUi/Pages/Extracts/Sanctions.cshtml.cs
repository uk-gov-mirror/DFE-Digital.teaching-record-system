...
public class Sanctions(TrsDbContext context, TimeProvider timeProvider) : PageModel
{
    ...
    public async Task<IActionResult> OnGetAudityAlertsAsync()
    {
        var rows = await context.Database
            .SqlQuery<AuditingAlertExtractRow>(
                $"""
                 ...
                 WHERE
                     a.updated_on > TIMESTAMPTZ '2025-07-31T23:59:59Z'
                     AND a.updated_on < TIMESTAMPTZ '2025-11-01T00:00:00Z'
                     AND atype.dqt_sanction_code NOT IN ('E3')
                     AND e.event_name IN ('AlertCreatedEvent', 'AlertDeletedEvent', 'AlertUpdatedEvent')
                ...
                $"""
                 ...
                 FROM persons p
                     ...
                     LEFT OUTER JOIN alert_types t
                         ON a.alert_type_id = t.alert_type_id
                 WHERE
                     a.deleted_on >= TIMESTAMPTZ '2025-09-17T00:00:00Z'
                     AND p.status = 0
            ...
            .SqlQuery<MonthlyTmuAlertReconciliationExtractRow>(
                $"""
                 ...
                 WHERE
                     p.status = 0
                     AND a.deleted_on IS NULL
                     AND a.start_date >= TIMESTAMPTZ '2019-01-01T00:00:00Z'
            ...
            .SqlQuery<MonthlyTmuAlertReconciliationKpiProcessExtractRow>(
                $"""
                 ...
                 WHERE
                     ...
                     AND p.trn IS NOT NULL
                     AND a.deleted_on IS NULL
                     AND a.start_date >= TIMESTAMPTZ '2020-01-01T00:00:00Z'
    ...
    public sealed class AuditingAlertExtractRowMap : ClassMap<AuditingAlertExtractRow>
    {
        public AuditingAlertExtractRowMap()
        {
            ...
            Map(m => m.EventType).Name("Event Type").Optional();
